...
df = pd.read_excel(r'C:\Users\Saarthak\Desktop\datasets\Delhi 2017-2024 Load\Wind_data.xlsx')
...
plt.plot(df['Datetime'],df['Actual_Wind_Speed'])
# plt.show()
for i in range(30):
    ...
    # plot_acf(train['Actual_Wind_Speed'].dropna(), lags=1000, ax=ax[0])
    # plot_pacf(train['Actual_Wind_Speed'].dropna(), lags=1000, ax=ax[1])

    # plt.show()
    print(train)
    print(test)

    model = SARIMAX(endog=train['Actual_Wind_Speed'],
                    order=(1, 1, 0),
    ...
    forecast_df = forecast_df.loc[train_end + pd.Timedelta(hours=17):test_end]
    test = test.loc[train_end + pd.Timedelta(hours=17):test_end]
    print(forecast_df.to_string())
    print(test.to_string())
    mape = mean_absolute_percentage_error(test['Actual_Wind_Speed'], forecast_df['Forecast'])
    print(f'mape:', mape * 100)

    result_df = pd.DataFrame({'Test': test['Actual_Wind_Speed'], 'Forecast': forecast_df['Forecast']})
    result_df.to_excel(
        f'C:/Users/Saarthak/Desktop/datasets/Delhi 2017-2024 Load/wind/result_i{i + 1}.xlsx',
        index=True)

    # plt.figure(figsize=(12, 6))
    # plt.plot(train.index, train['Actual_Wind_Speed'], label='train')
    # plt.plot(test.index, test['Actual_Wind_Speed'], label='test')
